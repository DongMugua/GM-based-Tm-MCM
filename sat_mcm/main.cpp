...
int main(int argc, char** argv) {
	...
	bool allow_negative_numbers = true;//允许加法器结果为负
	std::string solver_name = "none_solver";
	int threads = 16;
	bool also_minimize_full_adders = true;
	bool allow_node_output_shift = true;//加法后右移
	bool write_cnf = false;
	bool enumerate_all = true; //遍历所有可能得结果，不追求加法器数量最少
	int allow_coefficient_sign_inversion = 0;
	int min_num_adders = 0;
#ifdef USE_Z3
	solver_name = "z3";
#endif
#ifdef USE_SYRUP
