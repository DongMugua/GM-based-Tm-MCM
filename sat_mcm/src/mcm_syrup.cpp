...
void mcm_syrup::reset_backend(formulation_mode mode) {
	...
	// 创建 Glucose::ParallelSolver 实例
    Glucose::ParallelSolver* parallelSolver = new Glucose::ParallelSolver(this->threads);
	//this->solver = std::make_unique<Glucose::MultiSolvers>(this->threads);
	this->solver = std::make_unique<Glucose::MultiSolvers>(parallelSolver);
