...
void mcm::print_solution() {
	if (this->found_solution) {
		...
		// }
		//################################## ne
		string s = this->get_adder_graph_description();
		PAGSuite::adder_graph_t adder_graph;
		bool validParse = adder_graph.parse_to_graph(s);
		adder_graph.print_graph(constant);

		// for(PAGSuite::adder_graph_base_node_t* node : adder_graph.nodes_list){
		// 	// 使用 node_to_string 将节点转换为字符串
		...
		//std::cerr << "Adder graph: " << s << std::endl;
	}
	else {
		for (auto &c : this->C) {
...
void mcm::solve_enumeration() {
	...
	//std::cout<<"初始化计数器"<<solution_count<<std::endl;
	...
	while (this->found_solution) {
		...
		// 增加解决方案计数器
        solution_count++;
		//std::cout << "矩阵个数: " << solution_count << std::endl;
	...
	this->found_solution = true;
	// 输出解决方案总数
	std::cout << "矩阵个数: " << num_matrix << std::endl;
}
